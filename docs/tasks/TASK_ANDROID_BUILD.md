...
1. **Android SDK**: Install via Android Studio or download from https://developer.android.com/studio

2. **Android NDK**: Install via Android Studio SDK Manager:

   - Open Android Studio → Settings → SDK Manager → SDK Tools tab
   - Check "NDK (Side by side)" and install version 26.x or later
   - After install, update `android/app/build.gradle.kts`:
     ```kotlin
     ndkVersion = "26.1.10909125"  // Use your installed version
     ```

3. **local.properties**: Create `android/local.properties` with:
   ```properties
